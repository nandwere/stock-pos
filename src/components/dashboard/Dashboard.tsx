...
function ChangeBadge({ value }: { value: number | undefined }) {
  ...
  return (
    <div className={`flex items-center gap-1 text-sm font-medium ${value > 0 ? 'text-green-600' : 'text-red-600'}`}>
      <TrendingUp className={`w-4 h-4 ${value < 0 ? 'rotate-180' : ''}`} />
...
export default function Dashboard({ user }: any) {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* Stats Grid — 6 cards now, 3 per row */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow border border-gray-200">
          <div className="flex items-center justify-between mb-4">
            ...
            <ChangeBadge value={stats?.salesChange} />
          </div>
          <div>
          ...
          <>
            <div className="bg-white p-6 rounded-lg shadow border border-gray-200">
              <div className="flex items-center justify-between mb-4">
                ...
                <ChangeBadge value={stats?.profitChange} />
              </div>
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  {stats?.todaySales
                    ? `${(((stats.todayProfit || 0) / stats.todaySales) * 100).toFixed(1)}% margin`
                    : 'no sales yet'}
                </p>
              </div>
            </div>
            ...
            {/* Stock Profit — potential profit if current inventory sold at current prices */}
            <div className="bg-white p-6 rounded-lg shadow border border-gray-200">
              <div className="flex items-center justify-between mb-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {formatCurrency(stats?.stockProfit || 0)}
                </p>
                <p className="text-xs text-gray-500 mt-1">if all stock sold at current prices</p>
              </div>
            </div>
            {/* Active Workers */}
            <div className="bg-white p-6 rounded-lg shadow border border-gray-200">
